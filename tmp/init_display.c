...
/*   Updated: 2019/09/10 17:06:22 by ltimsit-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//int		get_carriage_color(t_arena *)
...
int		print_hexa_dis(t_arena *arena, t_display *dis, int index)
{
	...
	if ((arena->carriage[index] >> 4) & 1)
	{
		...
		if (k == 4)
			mlx_put_image_to_window(dis->mlx, dis->win, dis->img[k], col, line);
//		mlx_put_image_to_window(dis->mlx, dis->win, dis->img[arena->carriage[index] & 15], col, line);
	}
